...
#aliases
 ...
 alias colorconfig='nvim ~/.config/colors.sh'
 alias spfconfig='nvim ~/Library/Application\ Support/superfile/config.toml'
 ...
 alias clock='tty-clock -c -B -C 1 -s'
...
#function 
  ...
  function Run() {brew services start sketchybar && yabai --start-service && brew services start borders && skhd --start-service && /usr/sbin/networksetup -setairportpower Wi-Fi on }
  function sleepmode() {brew services stop sketchybar &&  yabai --stop-service && brew services stop borders && skhd --stop-service &&  sudo pmset -a ttyskeepawake 0; sudo pmset -a powernap 0; sudo pmset -a tcpkeepalive 0; sudo pmset -a womp 0 ; sudo pmset -b standby 0
sudo pmset -a standbydelayhigh 3600 # set to hibernate after 1 hour
/usr/sbin/networksetup -setairportpower Wi-Fi off
pmset sleepnow && osascript -e 'quit app "Iterm"'}
#function End

#zoxide Configure
eval "$(zoxide init --cmd cd zsh)"
#zoxide Configure End
eval "$(oh-my-posh init zsh --config ~/.config/theme/gruvbox.omp.json)"
